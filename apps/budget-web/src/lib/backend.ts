...
/** Forward real client UA/IP/geo so budget-backend sees the browser, not the BFF pod. */
...
async function callBudget(
  ...
  // trips a strict JSON body-parser on bodyless mutations (see edu-web's same fix).
  const res = await fetch(`${serverEnv.budgetApiBase}${path}`, {
    method,
...
/**
 * Proxy an authenticated request to budget-backend. Mints/refreshes the access token from
 * the shared Outegro refresh cookie (SSO), retrying once on 401 and persisting the rotated
 * pair.
 */
export async function proxyBudget(
  req: NextRequest,
  path: string,
  method: Method = "GET",
  body?: unknown,
  ...
  let result: BackendResult = access
    ? await callBudget(req, path, method, access, body)
  ...
  if (result.status === 401) {
    ...
    if (refresh) {
      ...
      if (rr.status >= 200 && rr.status < 300 && t?.accessToken && t.refreshToken) {
        ...
        result = await callBudget(req, path, method, refreshed.accessToken, body);
      }
    }
  }
