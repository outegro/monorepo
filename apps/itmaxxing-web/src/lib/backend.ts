...
/** Forward real client UA/IP/geo so itmaxxing-backend sees the browser, not the BFF pod. */
...
async function callItmaxxing(
  ...
  // Omit the body entirely when there's nothing to send — a "null" string body trips a
  // strict JSON parser on bodyless mutations.
  const res = await fetch(`${serverEnv.itmaxxingApiBase}${path}`, {
    method,
...
/**
 * Proxy an authenticated request to itmaxxing-backend. Mints/refreshes the access token from
 * the shared Outegro refresh cookie (SSO), retrying once on 401 and persisting the rotated pair.
 */
export async function proxyItmaxxing(
  req: NextRequest,
  path: string,
  method: Method = "GET",
  body?: unknown,
  ...
  let result: BackendResult = access
    ? await callItmaxxing(req, path, method, access, body)
  ...
  if (result.status === 401) {
    ...
    if (refresh) {
      ...
      if (rr.status >= 200 && rr.status < 300 && t?.accessToken && t.refreshToken) {
        ...
        result = await callItmaxxing(req, path, method, refreshed.accessToken, body);
      }
    }
  }
