...
/** Forward real client UA/IP/geo so trips-backend sees the browser, not the BFF pod. */
...
async function callItmaxxing(
  ...
  const res = await fetch(`${serverEnv.tripsApiBase}${path}`, {
    method,
...
/**
 * Proxy an authenticated request to trips-backend. Mints/refreshes the access token from
 * the shared Outegro refresh cookie (SSO), retrying once on 401 and persisting the rotated pair.
 */
export async function proxyTrips(
  req: NextRequest,
  path: string,
  method: Method = "GET",
  body?: unknown,
